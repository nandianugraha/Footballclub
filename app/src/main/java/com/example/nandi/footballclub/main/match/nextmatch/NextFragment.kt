package com.example.nandi.footballclub.main.match.nextmatch
...
class NextFragment : Fragment(), AnkoComponent<Context>, MatchView {
            ...
            override fun onItemSelected(parent: AdapterView<*>?, view: View?, position: Int, id: Long) {
                ...
                when (leagueName) {
                    getString(R.string.epl) -> {
                        presenter.loadNextMatch(getString(R.string.id_epl))
                    }
                    getString(R.string.laliga) -> {
                        presenter.loadNextMatch(getString(R.string.id_laliga))
                    }
                    getString(R.string.bundesliga) -> {
                        presenter.loadNextMatch(getString(R.string.id_bundesliga))
                    }
                    getString(R.string.seriea) -> {
                        presenter.loadNextMatch(getString(R.string.id_serie_a))
                    }
                    getString(R.string.ligue1) -> {
                        presenter.loadNextMatch(getString(R.string.id_ligue1))
                    }
                }
            }
        }
